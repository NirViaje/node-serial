...
void delaySleep(int m);

//#define delay(m) delaySleep(m)
...
void delaySleep(int m) {
  currMillis = millis();
  if(m > 2) {
//    Serial.print("Now delay for " + String(m) + "ms, current millis: " + String(millis()) + "ms.");
    m--;
    esp_sleep_enable_timer_wakeup(m*1000);
    int ret = esp_light_sleep_start();
    delay(1);
  } else {
    delay(m);
  }
//  Serial.println("\tTime elaspe: " + String(millis() - currMillis) +"ms.");
}
...
void IMUTest()
{
    ...
    while ((!M5.BtnA.isPressed()) && (!M5.BtnB.isPressed()))
    {
        ...
        const float scaleAvg = 0.0015625; //0.0625;
        accX = accXF*scaleAvg;
        accY = accYF*scaleAvg;
        accZ = accZF*scaleAvg;
...
void loop()
{
    ...
    //ESP.deepSleep(0 * SECONDS_TO_US);
//    esp_sleep_enable_timer_wakeup(100000); //0.1 seconds
//    Serial.println(String(millis()) + "ms, iSleep: " + String(iSleep++) + ", light_sleep_enter");
}
