...
#define XJT_VALID(i) (i->level0 && !i->level1 && i->duration0 >= 420 && i->duration0 <= 600)
...
void parseRmt(rmt_data_t* items, size_t len, uint32_t* channels){
    ...
    Serial.printf("rmt_data_t len: %d \n", len);

    if (!channels)  {
//        log_e
        Serial.println("Please provide data block for storing channel info");
        return;
    }
    s_channels = channels;
...
//        if(!valid)
        {
            Serial.printf("parseRmt valid (%d) at: %d\n", valid, i);
//            break;
        }
        it = &items[i];
        Serial.print(it->level0);  Serial.print(", "); Serial.print(it->level1);  Serial.print(", "); 
        Serial.print(it->duration0); Serial.print(", "); Serial.println(it->duration1);
        if(XJT_VALID(it)){
            if(it->duration1 >= 240 && it->duration1 <= 360){
                valid = xjtReceiveBit(i, false);
                Serial.printf("xjtReceiveBit %d, %d\n", i, false);
            } else if(it->duration1 >= 1000 && it->duration1 <= 1300){
                valid = xjtReceiveBit(i, true);
                Serial.printf("xjtReceiveBit %d, %d\n", i, true);
            } else {
                valid = false;
                Serial.printf("valid = false\n");
            }
        } else if(!it->duration1 && !it->level1 && it->duration0 >= 420 && it->duration0 <= 600) {
                    valid = xjtReceiveBit(i, false);
                    Serial.printf("elseif xjtReceiveBit %d, %d\n", i, false);
...
//    Serial.println("receive_data");
    Serial.print(data[0], BIN); Serial.print(", "); Serial.println(data[1], BIN);
...
void setup() 
{
    ...
    if ((rmt_recv = rmtInit(26, false, RMT_MEM_192)) == NULL)
    {
